...
// uint8_t tempStr[20]; //////你好你好
// sprintf((char *)tempStr, "RTOS_TASK   Total tables ");
// lcd_show_str(10, 10 + 20 * 3, 16, (char *)tempStr, DARKBLUE);
...
/******************************************************************************
      ...
      返回值：  无
******************************************************************************/
void lcd_close_BL(void)
{
    __HAL_TIM_SetCompare(&htim12, TIM_CHANNEL_2, 0);
    HAL_TIM_PWM_Stop(&htim12, TIM_CHANNEL_2);
}

void lcd_open_BL(void)
{
    HAL_TIM_PWM_Start(&htim12, TIM_CHANNEL_2);
    __HAL_TIM_SetCompare(&htim12, TIM_CHANNEL_2, 999);
}
/******************************************************************************
      函数说明：LCD调节背光
      入口数据：dc,占空比,1%~100%
      返回值：  无
******************************************************************************/
void lcd_set_BL(uint8_t dc)
{
    // 限制dc在0-100范围内
    if (dc > 100)
        dc = 100;
    // 将0-100映射到0-999
    uint32_t pulse = (dc * 999) / 100;
    lcd_open_BL();
